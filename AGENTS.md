...
3. Create or switch to an appropriate branch:
   ...
   - Otherwise create a clear branch name based on the task, such as `feature/bookmark-dedupe` or `fix/auth-redirect`.
4. Identify the smallest complete solution that satisfies the task.
5. If the task touches UI, layout, styling, navigation, or components, read `docs/design-system.md` before coding.
6. If the task touches Convex code, read `src/convex/_generated/ai/guidelines.md` before coding.
7. Inspect existing patterns before adding new ones.
7. Create a short PRD or implementation brief and save it in the appropriate docs folder before implementation.
8. For complex work, include:
   - Problem
   - Scope
   - Out of scope
   - Acceptance criteria
...
Do not begin implementation until the branch and brief are in place, unless the task is clearly a tiny fix.
...
- Preserve compact Linear-inspired density.
- Add loading, empty, error, and focus states where relevant.
- Add or update tests when touching pure logic, parsing, dedupe, search, AI output handling, or bug fixes.
...
6. Provide a clear summary breakdown of the changes made.
7. Mention:
